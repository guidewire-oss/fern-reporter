...
func (h *ProjectHandler) CreateProject(c *gin.Context) {
	...
	if err := h.db.Where("name = ?", project.Name).First(&project).Error; err == nil {
		utils.Log.Warn(fmt.Sprintf("[REQUEST-ERROR]: Project Name %s already exists for %s at %s", project.Name, method, path))
		c.JSON(http.StatusBadRequest, gin.H{"error": "Project Name already exists"})
		return
	} else if !errors.Is(err, gorm.ErrRecordNotFound) {
		utils.Log.Error(fmt.Sprintf("[ERROR]: Failed to query project name for %s at %s", method, path), err)
	...
	if err := h.db.Clauses(clause.Returning{}).Create(&project).Error; err != nil {
		utils.Log.Error(fmt.Sprintf("[ERROR]: Failed to create project %s for %s at %s", project.UUID, method, path), err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to create project"})
		return
	}
	utils.Log.Info(fmt.Sprintf("[REQUEST-SUCCESS]: Project %s created successfully for %s at %s", project.UUID, method, path))
...
func (h *ProjectHandler) UpdateProject(c *gin.Context) {
	...
	if err := h.db.Where("uuid = ?", id).First(&existing).Error; err != nil {
		utils.Log.Warn(fmt.Sprintf("[REQUEST-ERROR]: Project with UUID %s not found for %s at %s", id, method, path))
	...
	if err := h.db.Model(&models.ProjectDetails{}).
		...
		utils.Log.Error(fmt.Sprintf("[ERROR]: Database error for Project %s while checking for duplicates for %s at %s", project.UUID, method, path), err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Database error while checking for duplicates"})
		return
	}
	if count > 0 {
		utils.Log.Warn(fmt.Sprintf("[REQUEST-ERROR]: Project %s already exists for %s at %s", project.UUID, method, path))
		c.JSON(http.StatusBadRequest, gin.H{"error": "Project name already exists"})
		return
	}
	...
	if err := h.db.Save(&project).Error; err != nil {
		utils.Log.Error(fmt.Sprintf("[ERROR]: Failed to update project %s for %s at %s", project.UUID, method, path), err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to update project"})
		return
	}

	utils.Log.Info(fmt.Sprintf("[REQUEST-SUCCESS]: Project %s updated successfully for %s at %s", project.UUID, method, path))
...
func (h *ProjectHandler) DeleteProject(c *gin.Context) {
	...
	if err := h.db.Where("uuid = ?", uuid).Delete(&models.ProjectDetails{}).Error; err != nil {
		utils.Log.Error(fmt.Sprintf("[ERROR]: Failed to delete project %s for %s at %s", uuid, method, path), err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to delete project"})
		return
	}

	utils.Log.Info(fmt.Sprintf("[REQUEST-SUCCESS]: Project %s deleted successfully for %s at %s", uuid, method, path))
...
func (h *ProjectHandler) GetAllProjects(c *gin.Context) {
	...
	if err := h.db.Order("name ASC").Find(&projects).Error; err != nil {
		utils.Log.Error(fmt.Sprintf("[ERROR]: Failed to fetch projects for %s at %s", method, path), err)
