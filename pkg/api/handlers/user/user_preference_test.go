...
var _ = Describe("User Preference Handlers", Ordered, func() {
	...
	Context("when get favourite project is invoked", func() {
		It("will return favourite project list and return 200 OK", func() {
			projectUUIDs := []string{"96ad860-2a9a-504f-8861-aeafd0b2ae29", "59e06cf8-f390-5093-af2e-3685be593a25"}
			...
			Expect(response["cookie"]).To(Equal(ucookie))

			actualUUIDs := []string{}
			for _, uuid := range response["favourite_project_uuids"].([]interface{}) {
				actualUUIDs = append(actualUUIDs, uuid.(string))
			}
		
			Expect(actualUUIDs).To(Equal(projectUUIDs))
		})
