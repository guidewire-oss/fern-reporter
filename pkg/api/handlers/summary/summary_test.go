...
func setupTestEnvWithTaggedSpecs(groups []TestRunSpecGroup) testEnv {
	...
	api.Group("/reports/summary").GET("/:projectId/:seed", handler.GetSummary)
...
var _ = Describe("GetSummary", func() {
	...
	It("returns test summary for a valid project and seed", func() {
		...
		url := fmt.Sprintf("/api/reports/summary/%s/1693412583?group_by=testtype&group_by=component&group_by=owner&group_by=category", env.project.UUID)
	...
	It("returns test summary appropriately when grouping by something other than the default tags - eg just test type", func() {
		...
		url := fmt.Sprintf("/api/reports/summary/%s/1693412583?group_by=testtype", env.project.UUID)
		req, _ := http.NewRequest(http.MethodGet, url, nil)
		rec := httptest.NewRecorder()
	...
	It("returns test summary appropriately when grouping by two tags, test type and component", func() {
		...
		url := fmt.Sprintf("/api/reports/summary/%s/1693412583?group_by=testtype&group_by=component", env.project.UUID)
		req, _ := http.NewRequest(http.MethodGet, url, nil)
		rec := httptest.NewRecorder()
	...
	It("behaves logically when you ask it to group by a non-existent tag", func() {
		...
		url := fmt.Sprintf("/api/reports/summary/%s/1693412583?group_by=banana", env.project.UUID)
		req, _ := http.NewRequest(http.MethodGet, url, nil)
		rec := httptest.NewRecorder()
	...
	It("returns test summary appropriately when group_by clause is not specified", func() {
		...
		url := fmt.Sprintf("/api/reports/summary/%s/1693412583", env.project.UUID)
		req, _ := http.NewRequest(http.MethodGet, url, nil)
		rec := httptest.NewRecorder()
